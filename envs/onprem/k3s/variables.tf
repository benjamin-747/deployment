...
variable "app_resources" {
  ...
  description = "Per-service requests/limits/replicas override (keys: mono-engine, mega-ui, mega-web-sync, orion-server, campsite-api). Overriding a key replaces its whole object, so supply all fields."
  default     = {}
}
